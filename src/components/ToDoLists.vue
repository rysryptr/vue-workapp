...
const sortTasks = computed(() => {
  const progressOrder = {1: 0, 0: 1, 2: 2}

  return [...state.tasks].sort((a, b) => {

    const progressA = progressOrder[a.taskProgress]
    const progressB = progressOrder[b.taskProgress]

    if(progressA !== progressB) {
      return progressA - progressB
    }

    return b.taskSeverity - a.taskSeverity
  })
})
...
const sortedByActions = computed(() => {
  if(!sortKey.value) return sortTasks.value
  ...
  <div class="bg-white max-w-7xl w-full mx-auto rounded-xl p-8 mt-4">
    ...
    <div v-if="state.tasks.length" class="flex flex-col mt-8 p-4 border border-gray-200 rounded-3xl">
      <div class="flex text-center mb-2">
        ...
        <div 
          class="w-1/6 font-bold flex gap-2 hover:cursor-pointer" 
          @click="setKey('taskSeverity')">
            Task Severity
            <span v-if="sortOrder === ''"></span>
            <Icon v-else-if="sortOrder === 1 && sortKey === 'taskSeverity'" icon="bxs:up-arrow" />
            <Icon v-else-if="sortOrder === -1 && sortKey === 'taskSeverity'" icon="bxs:down-arrow" />
        </div>
        <div class="w-1/6 font-bold">Task Time</div>
        <div class="w-1/6 font-bold">Actual Time</div>
        <div 
          class="w-1/6 font-bold flex gap-2 hover:cursor-pointer" 
          @click="setKey('taskProgress')"
        >
          Task Progress
          <span v-if="sortOrder === ''"></span>
          <Icon v-else-if="sortOrder === 1 && sortKey === 'taskProgress'" icon="bxs:up-arrow" />
          <Icon v-else-if="sortOrder === -1 && sortKey === 'taskProgress'" icon="bxs:down-arrow" />
        </div>
        <div class="w-1/6 font-bold">Actions</div>
      </div>
      <ToDoList 
        v-for="task in sortedByActions" 
        :key="task.taskId" 
        :task="task" 
